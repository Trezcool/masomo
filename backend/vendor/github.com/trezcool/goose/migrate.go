...
func CollectMigrations(fsys fs.FS, dirpath string, current, target int64) (Migrations, error) {
	var err error
	if fsys != nil {
		_, err = fs.Stat(fsys, dirpath)
	} else {
	...
	// Go migration files
	var goMigrationFiles []string
	if fsys != nil {
		goMigrationFiles, err = fs.Glob(fsys, dirpath+"/*.go")
	} else {
